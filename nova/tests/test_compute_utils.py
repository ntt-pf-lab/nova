...
class UtilsTestCase(test.TestCase):
    ...
    def test_terminate_volumes_database_volume_id_is_not_none_and_delete_on_termination_is_true(self):
        ...
        self.utils.terminate_volumes(db, self.context, 1)
        self.assertEqual(block_device_mappings[1]['volume_id'], self._volume_id)

    @attr(kind='small')
    def test_terminate_volumes_exception_volume_api_delete(self):
        """
        All BlockDeviceMappings are deleted even when exception is raised in volume_api.delete()
        """
        self._count = 0
        def stub_delete(caller, context, volume_id):
            self._count += 1
