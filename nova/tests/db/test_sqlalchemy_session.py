...
class SQLAlchemySessionTestCase(test.TestCase):
    ...
    def test_get_session_parametar(self):
        """ Ensure return a SQLAlchemy session when parametar is not default"""
        session = db_session.get_session(autocommit=False, expire_on_commit=True)
        self.assert_('sqlalchemy.orm.session.Session object'in str(session))

    @attr(kind='small')
    def test_get_engine_configuration_drivername_is_other(self):
        """ Ensure return a SQLAlchemy engine when drivername is not sqlite"""
